\begin{tikzpicture}
\begin{axis}[width=\marginparwidth+25pt,%
tick label style={font=\scriptsize},axis y line=middle,axis x line=middle,name=myplot,%
			%x=.37\marginparwidth,
			%y=.37\marginparwidth,
			%xtick={2,4,6,8,10},
			%minor x tick num=4,% 
%			extra x ticks={.33},
%			extra x tick labels={$1/3$},
			%ytick={-10,-5,5,10},
			%minor y tick num=4,%extra y ticks={-5,-3,...,7},%
			ymin=0,ymax=160,%
			xmin=0,xmax=210%
]

%\draw (axis cs:4,-1) node (A) {}
			%(axis cs:4,3) node (B) {} ;
%\draw [->,>=latex] (A) parabola bend (axis cs:0,1) (B);
\addplot [thick, {\colorone},smooth,domain=0:6] ({30*x},{-16*x^2+96*x});
\draw [->,>=latex] (axis cs:30,80) -- (axis cs:33, 86.24);
%\draw [->,>=latex] (axis cs:7,11) -- (axis cs:7.1107, 11.0601);
\filldraw (axis cs:60, 128) circle (1.5pt) node [above left] {\scriptsize $t=2$};

\draw (axis cs: 100,50) node [align=left] {\scriptsize $x=30t$\\ \scriptsize $y=-16t^2+96t$};



			

\end{axis}

\node [right] at (myplot.right of origin) {\scriptsize $x$};
\node [above] at (myplot.above origin) {\scriptsize $y$};
\end{tikzpicture}




















