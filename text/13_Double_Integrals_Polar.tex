...
\intertext{We can evaluate this inner integral with substitution. With $u=a^2-r^2$, $du = -2r\ dr$. The new bounds of integration are $u(0) = a^2$ to $u(a)=0$. Thus we have:}
	...
	&= \frac43\pi a^3.
\end{align*}
Generally, the formula for the volume of a sphere with radius $r$ is given as $\frac{4}{3}\pi r^3$; we have justified this formula with our calculation.  In Section~\ref{sec:spherical}, we will be able to use spherical coordinates to write a triple integral that is even easier to compute.
}\\
