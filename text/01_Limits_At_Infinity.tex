...
Consider again $f(x) = 1/x^2$, as shown in Figure \ref{fig:oneoverxsquared}. Note that as $x$ gets very large, $f(x)$ gets very, very small. We could represent this concept with notation such as $$\lim_{x\rightarrow \infty} \frac1{x^2}=0.$$

\mfigure{.8}{Graphing $f(x) = 1/x^2$.}{fig:oneoverxsquared}{figures/figoneoverxsquared}
