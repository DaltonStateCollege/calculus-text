...
In Section~\ref{sec:center_of_mass_calc_ii}, we introduced centers of mass, but only computed them for planar laminas with constant density.  With the tools of double integrals, we can now compute a broader range of centers of mass.
...
Find the mass of a square lamina, represented by the unit square with lower lefthand corner at the origin (see Figure \ref{fig:mass1}), with variable density $\delta(x,y) = (x+y+2)$g/cm$^2$.}
...
W &= \int_0^{2\pi}\int_0^2 (2r+1)r\ dr\ d\theta\\
	&= \int_0^{2\pi} \left(\frac23r^3+\frac12r^2\right)\Big|_0^2d\theta\\
	&= \int_0^{2\pi} \frac{22}3\ d\theta\\
...
Find the center mass of a square lamina, with side length 1, with a density of $\delta = 3$g/cm$^2$. (Note: this is the lamina from Example \ref{ex_mass1}.)}
...
Find the center of mass of a square lamina, represented by the unit square with lower lefthand corner at the origin (see Figure \ref{fig:mass6}), with variable density $\delta(x,y) = (x+y+2)$g/cm$^2$. (Note: this is the lamina from Example \ref{ex_mass2}.)}
