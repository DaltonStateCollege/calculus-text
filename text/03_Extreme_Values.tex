...
Note that all this was done without the aid of a graph; this work followed an analytic algorithm and did not depend on any visualization. Figure \ref{fig:extval4} shows $f$ and we can confirm our answer, but it is important to  understand that these answers can be found without graphical assistance.  In fact, if we simply looked at the graph to determine the extrema, it would not be clear that the minimum occurs at $x=1$ as opposed to $x=0.999$.  So we must use Key Idea~\ref{idea:extrema} to find these extrema.
