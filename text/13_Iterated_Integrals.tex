...
$$\int_c^d g(y) \left( \int_a^b f(x) dx \right) dy = \left( \int_a^b f(x) dx \right) \left(\int_c^d g(y) dy\right).$$

We use this shortcut in the next example.

\example{ex_double9}{Using Theorem~\ref{thm:dint_prod_shortcut}}
