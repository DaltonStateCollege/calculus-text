...
\begin{enumerate}
	...
	\item		If $f$ has a relative maximum or minimum at $P$, then $f$ has a \textbf{relative extremum} at $P$; if $f$ has an absolute maximum or minimum at $P$, then $f$ has na \textbf{absolute extremum} at $P$.
\end{enumerate}
}
...
{Let $z=f(x,y)$ be defined on an open set $S$ containing $P=(x_0,y_0)$. If $f$ has a relative extremum at $P$, then $P$ is a critical point of $f$.\index{extrema!relative}\index{critical point}
}
...
\theorem{thm:multi_second_test}{Second Derivative Test for Functions of Two Variables}
...
\end{enumerate}
\textbf{Note: }We can substitute $f_{xx}(x_0,y_0)$ with $f_{yy}(x_0,y_0)$ in (1) and (2) above.  If $D>0$, then $f_{xx}(x_0,y_0)$ and $f_{yy}(x_0,y_0)$ are either both positive or both negative.
}
...
Finally, we evaluate $f$ along the right edge of the triangle, where $y = -\frac32 x+1$. 
$$f(x,y) = f\left(x,-\frac32 x+1\right) = x^2-\left(-\frac32 x+1\right)^2+5 = -\frac54x^2+3x+4=f_3(x).$$
