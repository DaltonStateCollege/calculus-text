...
{\begin{enumerate}
	\item Let $\vec u = \la u_1,u_2\ra$ and $\vec v = \la v_1,v_2\ra$ be vectors in $\mathbb{R}^2$, and let $c$ be a \textbf{scalar} (a real number or a variable expression that represents a real number). \index{vectors!algebra of}
				\begin{enumerate}
...
For any vector $\vec v$ in $\mathbb{R}^2$ or $\mathbb{R}^3$, the vector $(-1) \vec v$ is denoted by $-\vec v$.  The vector $-\vec v$ has the same magnitude as $\vec v$ but points in the \emph{opposite} direction.
...
\begin{enumerate}
	...
	\item \parbox{150pt}{$c(\vec u\pm\vec v) = c\vec u\pm c\vec v$}Distributive Property
	\item \parbox{150pt}{$(c\pm d)\vec v = c\vec v\pm d\vec v$}Distributive Property
	\item \parbox{150pt}{$0\vec v = \vec 0$}
	\item	\parbox{150pt}{$\norm{c\vec v} = |c|\cdot\norm{\vec v}$}\label{thm:norm_prop}
	\item	$\vnorm u = 0$ if and only if $\vec u = \vec 0$.  \label{thm:zero_norm}
\end{enumerate}
}
