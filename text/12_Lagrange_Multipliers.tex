...
\example{ex_lagrange1}{}{Minimize $f(x,y) = x^2 + y^2$ subject to the restraint $g(x,y) = 2x + y = k$, for a constant $k$.}{Look at the level curves in Figure \ref{}. They are circles $$x^2 + y^2 = c.$$ When $c$ is small, the circles do not touch the line $2x + y = k$. There are no points that satisfy the constraint, when $c$ is too small. Now increase $c$. Eventually the growing circles $x^2 + y^2 = c$ will just touch the line $x + 2y = k$. The point where they touch is the winner. It gives the smallest value of $c$ that can be achieved on the line. The touching point is $(x_{\text{min}}, y_{\text{min}})$, and the value of $c$ there is $f_{\text{min}}$.
...
\example{ex_lagrange4}{}{Find the minimum value of $f(x,y,z) = x^2 + y^2 + z^2$ when $$g = x+y+z = 9 \: \text{ and } \: h = x+2y+3z = 20.$$}{In figure \ref{}, the normals to those planes are $\nabla g = (1,1,1)$ and $\nabla h = (1,2,3)$.  The gradient of $f$ is $\nabla f = (2x,2y,2z)$.  The equation $$\nabla f = \lambda_1 \nabla g + \lambda_2 \nabla h$$ becomes
