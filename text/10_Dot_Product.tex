...
\begin{enumerate}
	...
	\item \parbox{150pt}{$\vec u\cdot(\vec v\pm \vec w) = \dotp uv \pm \dotp uw$}{Distributive Property}
	\item \parbox{150pt}{$(\vec u\pm \vec v) \cdot \vec w = \dotp uw \pm \dotp vw$}{Distributive Property}
	\item	$c(\dotp uv) = (c\vec u)\cdot \vec v = \vec u \cdot (c\vec v)$
	\item	$\dotp 0v = \dotp v0 = 0$
	\item	$\dotp vv=\norm{\vec v}^2 $
\end{enumerate}
}
