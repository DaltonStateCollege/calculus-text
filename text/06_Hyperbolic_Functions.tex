\section{Hyperbolic Functions (Optional)}\label{sec:hyperbolic}
...
\begin{enumerate}
\item		$\ds \sinh x = \frac{e^x-e^{-x}}2$\index{hyperbolic function!definition}
...
\begin{tabular}{ccc}
Function & Domain & Range\\ \hline
\rule{0pt}{10pt}$\sinh^{-1} x$ & $(-\infty,\infty)$ & $(-\infty,\infty)$\\
...
\begin{enumerate}
\item $\ds\sinh^{-1}x = \ln\big(x+\sqrt{x^2+1}\big)$\index{hyperbolic function!inverse!logarithmic def.}\rule[-10pt]{0pt}{20pt}
...
\begin{enumerate}
\item $\ds\frac{d}{dx}\big(\sinh^{-1} x\big) = \frac{1}{\sqrt{x^2+1}}$\index{derivative!inverse hyper.}\index{hyperbolic function!inverse!derivative}
...
{%
\begin{enumerate}
\item \parbox{70pt}{$\ds\int \frac{1}{\sqrt{x^2+a^2}}\ dx$} \parbox{180pt}{$\ds=\qquad \sinh^{-1}\left(\frac xa\right)+C;\ a>0$} $\ds=\ln\Big|x+\sqrt{x^2+a^2}\Big|+C$\index{integration!inverse hyper.}\index{hyperbolic function!inverse!integration}
...
\item		This requires a substitution, then item \#1 of Key Idea \ref{idea:hyperbolic_inverse_integrals} can be applied.
