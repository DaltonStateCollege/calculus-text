...
The \textit{floor function},\index{floor function} $f(x) = \lfloor x \rfloor$, returns the largest integer less than the input $x$. (For example, $f(\pi) = \lfloor \pi \rfloor = 3$.) The graph of $f$ in Figure \ref{fig:continuous2} demonstrates why this is often called a ``step function.'' \\
...
\item		$\ds f(x) =  \log_a x$ ($a>0, a\not=1$)\addtocounter{enumi}{1}
\item		$\ds f(x) =  a^x$ ($a>0$)
\end{enumerate}
\end{minipage}
...
{We examine each in turn, applying Theorems \ref{thm:continuity_algebra} and \ref{thm:continuous_functions} as appropriate.
	...
	\mfigure{.7}{A graph of $f$ in Example \ref{ex_cont_funct}(1).}{fig_continuous3}{figures/figContinuous3}
			...
			\item		The domain of $y = \sqrt{x}$ is $[0,\infty)$. The range of $y=\ln x$ is $(-\infty,\infty)$, but if we restrict its domain to $[1,\infty)$ its range is $[0,\infty)$. So restricting $y = \ln x$ to the domain of $[1,\infty)$ restricts its output is $[0,\infty)$, on which $y = \sqrt{x}$ is defined. Thus the domain of $f(x) = \sqrt{\ln x}$ is $[1,\infty)$.  Using Theorem \ref{thm:continuous_functions} and the Theorem \ref{thm:continuity_algebra} for composition of functions, $f(x) = \sqrt{\ln x}$ is continuous on its domain of $[1,\infty)$
			\end{enumerate}
\vskip -\baselineskip
}\\
