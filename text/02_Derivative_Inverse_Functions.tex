...
We have discovered a relationship between $\fp$ and $g\primeskip'$ in a mostly graphical way. We can realize this relationship analytically as well. Let $y = g(x)$, where again $g = f\primeskip^{-1}$. We want to find $\ds y\primeskip'$. Since $y = g(x)$, we know that $f(y) = x$. Using the Chain Rule and Implicit Differentiation, take the derivative of both sides of this last equality.
		\begin{align*}
			...
			\fp(y)\cdot \frac{dy}{dx} &= 1\\
			\frac{dy}{dx} &= \frac{1}{\fp(y)}\\
			\frac{dy}{dx} &= \frac{1}{\fp(g(x))}
		\end{align*}
		
This leads us to the following theorem.

\setboxwidth{110pt}
\noindent\hskip-110pt\begin{minipage}{1.2\linewidth}
...
\hskip-7pt	\begin{tabular}{ccc}
	$\ds \left(f\primeskip^{-1}\right)'(b)=g\primeskip'(b) = \frac{1}{\fp(a)}$ &\hskip 4pt and \hskip 4pt&  $\ds \left(f\primeskip^{-1}\right)'(x)=g\primeskip'(x) = \frac{1}{\fp(g(x))}=\frac{1}{\fp\big(f\primeskip^{-1}(x)\big)}.$
	\end{tabular}
%	\end{center}
}
\end{minipage}
...
%We restate the derivative rules in the following theorem, intended to be a reference for further work.

\theorem{thm:deriv_glossary}{Glossary of Derivatives of Elementary Functions}
{Let $u$ and $v$ be differentiable functions, and let $a$, $c$ and $n$ be real numbers, $a>0$. \\

\noindent%
\begin{enumerate}
	\item		$\frac{d}{dx}\big(cf(x)\big) = c\fp(x)$%\addtocounter{enumi}{1}
	\item		$\frac{d}{dx}\big(f(x)\pm g(x)\big) = \fp(x)\pm g\primeskip'(x)$%\addtocounter{enumi}{1}
	\item		$\frac{d}{dx}\big(f(x)g(x)\big) = \fp(x)g(x)+f(x)g\primeskip'(x)$%\addtocounter{enumi}{1}
	\item		$\frac{d}{dx}\big(\frac{f(x)}{g(x)}\big) = \frac{g(x)\fp(x)-f(x)g\primeskip'(x)}{g(x)^2}$%\addtocounter{enumi}{1}
\end{enumerate}
	\begin{minipage}{.5\specialboxlength}
	\begin{enumerate}\addtocounter{enumi}{4}
	\item		$\frac{d}{dx}\big(f(g(x))\big) = \fp(g(x)) g\primeskip'(x)$\addtocounter{enumi}{1}
...
\begin{minipage}{.5\specialboxlength}
	...
	\item		$\frac{d}{dx}\big(\log_a x\big) = \frac{1}{\ln(a)x}$\addtocounter{enumi}{1}
	...
	\item		$\frac{d}{dx}\big(\cot^{-1}x\big) = -\frac{1}{1+x^2}$\addtocounter{enumi}{1}
...
We restated the derivative rules in the previous theorem, intended to be a reference for further work.
There are a lot of rules to learn and remember for taking derivatives.  However, just like when learning how to spell or the meanings of vocabulary rules, etc.\ the best way to learn the derivative rules is to get used to them through practice.
