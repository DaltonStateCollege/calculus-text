...
$f \circ g$ and read it as ``$f$ of $g$.''  Before giving the corresponding differentiation rule, we note that the rule extends to multiple compositions like $f(g(h(x)))$ or $f(g(h(j(x))))$, etc.
...
\theorem{thm:gen_power_rule}{Generalized Power Rule}{Let $g(x)$ be a differentiable function and let $n$ be a real number.\index{derivative!Generalized Power Rule}\index{Generalized Power Rule} Then $$\frac{d}{dx}\Big(g(x)^n\Big) = n\cdot \big(g(x)\big)^{n-1}\cdot g\primeskip'(x).$$
}

This allows us to quickly find the derivative of functions like $$y = \left(3x^2-5x+7+\sin x\right)^{20}.$$ While it may look intimidating, the Generalized Power Rule states that $$y\primeskip' = 20\left(3x^2-5x+7+\sin x\right)^{19}\cdot (6x-5+\cos x).$$
...
%			\item		$y = \sin{2x}$\addtocounter{enumi}{1}
			\item		$y= \ln \left(4x^3-2x^2\right)$
			\end{enumerate}
			\end{minipage}
...
{\begin{enumerate}
		...
		\item		Recognize that $y = \ln \left(4x^3-2x^2\right)$ is the composition of $f(x) = \ln x$ and $g(x) = 4x^3-2x^2$. Also, recall that $$\frac{d}{dx}\Big(\ln x\Big) = \frac{1}{x}.$$ This leads us to:
		$$y\primeskip' = \frac{1}{4x^3-2x^2} \cdot \left(12x^2-4x\right) = \frac{12x^2-4x}{4x^3-2x^2}= \frac{4x(3x-1)}{2x(2x^2-x)} = \frac{2(3x-1)}{2x^2-x}.$$
...
A concrete example of this is $$\frac{d}{dx}\Big(\ln\big(3x^{15}-\cos x+e^x\big)\Big) = \frac{45x^{14}+\sin x+e^x}{3x^{15}-\cos x+e^x}.$$ While the derivative may  look intimidating at first, look for the pattern. The denominator is the same as what was inside the natural log function; the  numerator is simply its derivative.
...
\item		We must use the Product and Chain Rules. Do not think that you must be able to ``see'' the whole answer immediately; rather, just proceed step--by--step.
		$$\fp(x) = \big(6x^2\cos 2x^3\big)x^5 + 5x^4\big(\sin 2x^3\big)= 6x^7\cos2x^3+5x^4\sin 2x^3.$$
\item		We must employ the Quotient Rule along with the Chain Rule. Again, proceed step--by--step.
\begin{align*}
\fp(x) = \frac{e^{-x^2}\big(15x^2\big) - 5x^3\left((-2x)e^{-x^2}\right)}{\big(e^{-x^2}\big)^2} &=\frac{e^{-x^2}\big(10x^4+15x^2\big)}{e^{-2x^2}}\\
...
Find the derivative of $y = \tan^5\left(6x^3-7x\right)$.}
{Recognize that we have the $g(x)=\tan\left(6x^3-7x\right)$ function ``inside'' the $f(x)=x^5$ function; that is, we have $y = \left(\tan\left(6x^3-7x\right)\right)^5$. We begin using the Generalized Power Rule; in this first step, we do not fully compute the derivative. Rather, we are approaching this step--by--step.
$$y\primeskip' = 5\left(\tan\left(6x^3-7x\right)\right)^4\cdot g\primeskip'(x).$$
We now find $g\primeskip'(x)$. We again need the Chain Rule; $$g\primeskip'(x) = \sec^2\left(6x^3-7x\right)\cdot\left(18x^2-7\right).$$ Combine this with what we found above to give
\begin{align*}
y\primeskip' &= 5\left(\tan\left(6x^3-7x\right)\right)^4\cdot\sec^2\left(6x^3-7x\right)\cdot\left(18x^2-7\right)\\ 
&= \left(90x^2-35\right)\sec^2\left(6x^3-7x\right)\tan^4\left(6x^3-7x\right).
\end{align*}
...
The Chain Rule also has theoretic value.  In Section~\ref{sec:basic_diff_rules}, we stated that the derivative of $y=a^x$ is $y\primeskip'=\ln a\cdot a^x$.  However, we only proved the derivative for $e^x$ using the derivative definition. In the following example, we use the derivative rule for $y=e^x$ to prove the rule for $y=a^x$, where $a>0$.\\

\example{ex_chain8}{Using the Chain Rule to prove $\frac{d}{dx}(a^x)=\ln a \cdot a^x$}{
Use the Chain Rule to find the derivative of $y= a^x$ where $a>0$ is constant.}
{ This can be accomplished by rewriting $a^x$ in terms of $e$.  Recalling that $e^x$ and $\ln x$ are inverse functions, we can write
$$
a = e^{\ln a} \quad \text{and so } \quad y = a^x = \left(e^{\ln a}\right)^x = e^{x (\ln a)}.
$$
...
$$
y\primeskip' = e^{x (\ln a)} \cdot \ln a = a^x\cdot \ln a.
$$
}\\
