...
Despite this latter difficulty, there are still broad classes of functions for which we can find antiderivatives. This section and the following chapter are dedicated to learning techniques to enable us to compute the antiderivatives of a wide variety of functions.
...
\begin{align*}
	\int \frac{7}{-3x+1}\ dx &=	\int \frac{7}{u}\cdot\frac{du}{-3} \\
...
In this example, let's set $u = \sin x$. Then $du = \cos x\ dx$, which we have as part of the integrand! The substitution becomes very straightforward:
		...
		\int \sin x\cos x\ dx &=	\int u\ du \\
											...
											&= \frac12\sin^2 x + C.
		\end{align*}
One would do well to ask ``What would happen if we let $u = \cos x$?'' The result is just as easy to find, yet looks very different. The challenge to the reader is to evaluate the integral letting $u = \cos x$ and discover why the answer is the same, yet looks different.  This is explored in Exercise~\ref{exer:06_01_ex_99}.
}\\
...
The same type of trick can be used to help remember the antiderivatives of the six trigonometric functions as for the derivatives.  If one remembers the antiderivatives of $\sin x$, $\tan x$, and $\sec x$, then the antiderivative of the corresponding ``cofunction'' is found by changing between ``co-'' and ``not co-'', and toggling the minus sign in front.  For example, suppose we know the antiderivative of $\sec x$ is $\ln|\sec x+\tan x| + C$.  Then the antiderivative of $\csc x$ is formed by changing secant to cosecant, tangent to cotangent, and putting a minus sign in front.  Unlike derivative rules, however, note the ``cofunctions'' are \textbf{not} always the ones whose antiderivatives have the minus sign in the front.\\
...
The process we'll employ is to use a trigonometric identity for $\cos^2x$ (perhaps consult the back of this text for this formula), which states 
...
When dealing with rational functions (i.e., quotients made up of polynomial functions), it is an almost universal rule that everything works better when the degree of the numerator is less than the degree of the denominator. Hence we use polynomial division.  At each step, divide the leading terms.  We finish when the degree of the remainder is less than the degree of the divisor $x^2+2x+1$.
%Mike Joseph (6/10/2018) added this brief explanation and computation of the polynomial division.  The original text omitted the steps.
$$
\begin{array}{rlcccccccr}
&& x & + &2  \\ \cline{2-10}\vspace{-0.9em}\\
\multicolumn{1}{r|}{x^2+2x+1} && x^3 & +& 4x^2 &+  &8x  &+  &5 &  \\
&-( & x^3 & + & 2x^2 & + &  x &&&)\\ \cline{2-10}\vspace{-0.9em}\\
&& & & 2x^2 & + & 7x & +&5   \\ 
&-( &&  & 2x^2 &+ & 4x & + & 2 &)\\ \cline{2-10}\vspace{-0.9em}\\
&&&&&  & 3x & + & 3 
\end{array}
$$

So when $x^2+2x+1$ is divided into $x^3+4x^2+8x+5$, it goes in $x+2$ times with a remainder of $3x+3$. Thus 
...
\end{align*}
In some ways, we ``lucked out'' in that after dividing, substitution was able to be done. In Section~\ref{sec:partial_fraction} we'll develop techniques for handling rational functions where substitution is not directly feasible.
}\\
...
{% not longpage
\mtable{.68}{Graphing the areas defined by the definite integrals of Example \ref{ex_subst13}.}{fig:subst13}{\begin{tabular}{c}
