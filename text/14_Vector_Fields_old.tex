...
\mfigure[scale=0.5]{.7}{The vector field $\mathbf{R}$}{fig:fig_vec_fields_radial}{figures/fig_vec_fields_radial}

\mfigure[scale=0.5]{.5}{The vector field $\mathbf{R}$}{fig:fig_vec_fields_radial_over_r}{figures/fig_vec_fields_radial_over_r}

\mfigure[scale=0.5]{.3}{The vector field $\mathbf{R}$}{fig:fig_vec_fields_radial_over_r_squared}{figures/fig_vec_fields_radial_over_r_squared}
...
\example{vecfield1}{}{Consider the position vector field $\vec R = x \vec i + y \vec j$.}{Its components are $M(x,y) = x$ and $N(x,y) = y$. The vectors grow larger as we leave the origin - see Figure \ref{fig:fig_vec_fields_radial}. Their direction is outward and their length is $\mid R \mid = \sqrt{x^2+y^2} = r$. This type of vector field is often referred to as a radial field.}\\

\medskip

\example{vecfield2}{}{Now suppose the vector field $\vec F$ is given by $\vec F = \dfrac{\vec R}{r}$.}{Then $$\vec F(x,y) = \dfrac{x}{\sqrt{x^2 + y^2}} \vec i + \dfrac{y}{\sqrt{x^2 + y^2}} \vec j$$ consists of unit vectors pointing outward. We obtain this vector field by dividing $\vec R = x\vec i + y\vec j$ by its length at every point except the origin. See Figure \ref{fig:fig_vec_fields_radial_over_r}.  Figure \ref{fig:fig_vec_fields_radial_over_r_squared} shows a third vector field $\dfrac{\vec R}{r^2}$, where the length of each vector is $1/r$.}\\
...
\mfigure[scale=0.5]{.8}{The vector field $\mathbf{S}$}{fig:fig_vec_fields_spin}{figures/fig_vec_fields_spin}

\mfigure[scale=0.5]{.6}{The vector field $\mathbf{S}/r$}{fig:fig_vec_fields_spin_over_r}{figures/fig_vec_fields_spin_over_r}

\mfigure[scale=0.5]{.4}{The vector field $\mathbf{S}/r^2$}{fig:fig_vec_fields_spin_over_r_squared}{figures/fig_vec_fields_spin_over_r_squared}

as given in Figure \ref{fig:fig_vec_fields_spin} where the components are $M(x,y) = - y$ and $N(x,y) = x$. Each vector $\vec S(x,y)$ has length $$\mid \vec S \mid = \sqrt{(-y)^2 + x^2} = r$$
...
See Figure \ref{fig:fig_vec_fields_spin_over_r} and Figure \ref{fig:fig_vec_fields_spin_over_r_squared}.  Notice the blank at $(0,0)$ in these second two spin fields, where these fields are not defined.}\\
...
\example{vecfield3a}{}{Consider the function $f(x,y) = x^2y$.  Determine the gradient vector field associated to this function.}{The partial derivatives of $f(x,y)$ are $$\dfrac{\partial f}{\partial x} = 2xy \: \text{ and } \: \dfrac{\partial f}{\partial y} = x^2$$ and so the gradient vector field corresponding to $f(x,y)$ is given by
...
where $G$ is the gravitational constant. ($G \approx 6.672 \times 10^{-11}$ when distance is in meters, mass is in kilograms, and time is in seconds.)  Assume that the object with mass $M$ is at the origin in $\mathbb{R}^3$.
}{As a vector field we can write
...
\printexercises{exercises/14_01_exercises_old}