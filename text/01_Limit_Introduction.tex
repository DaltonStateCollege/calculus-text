...
Consider the function $y = \frac{\sin x}{x}$. When $x$ is near the value 1, what value (if any) is $y$ near?  (Unless otherwise stated, in calculus we always use \textit{radian} measures for defining trigonometric functions.)%
...
\emph{Finding a limit} entails understanding how a function behaves near a particular value of $x$. 

\keyidea{idea:limit}{Informal Definition of Limit}
{
The expression ``the limit of $f(x)$ as $x$ approaches the number $c$ equals $L$,'' denoted $\displaystyle \lim_{x \to c} f(x) = L$, means that as values for $x$ get closer and closer to the number $c$, the values of $f(x)$ get closer and closer to the number $L$. \index{limit!informal definition}
}
...
Above, where $f(x) = \sin(x)/x$, we can approximate $$\lim_{x\to 1} \frac{\sin x}{x} \approx 0.84 \quad \text{ and } \quad \lim_{x\to 0}\frac{\sin x}{x} \approx 1$$ using the tables in Figures~\ref{table:sinx_1} and~\ref{table:sinx_2}. (We \textit{approximated} these limits, hence used the ``$\approx$'' symbol, since we are working with the pseudo-definition of a limit, not the actual definition.)
...
We can deduce this on our own, without the aid of the graph and table. If $x$ is near 1, then $(x-1)^2$ is very small, and: $$\frac{1}{\text{very small number}} = \text{very large number}.$$ (We use the word \textit{small} to mean close to zero.)
