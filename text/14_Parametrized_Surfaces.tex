\section{Parametric Surfaces and Surface Area}\label{sec:parametric_surfaces}
...
\definition{def:parameterized_surface}{Parametric Surface}
{Let $\vec r(u,v) = \langle\, f(u,v),g(u,v),h(u,v)\rangle$ be a vector--valued function
that is continuous and one-to-one on the interior of its domain $R$ in the $u$-$v$ plane. The set of all terminal points of $\vec r$ (i.e., the \emph{range} of $\vec r$\,) is the \sword{surface} \surfaceS, and $\vec r$ along with its domain $R$ form a \sword{parameterization} of \surfaceS.\\

This parameterization is \sword{smooth} on $R$ if $\vec r_u$ and $\vec r_v$ are continuous and $\vec r_u\times \vec r_v$ is never $\vec 0$ on the interior of $R$.\index{surface}\index{parametric equations!of a surface}\index{parameterized surface}\index{smooth!surface}\index{surface!smooth}
}
...
3Dlights=Headlamp,add3Djscript=asylabels.js}{width=145pt}{.57}{The surface parameterized in Example \ref{ex_parsurf1}.}{fig:parsurf1}{figures/figparsurf1} 
...
3Dcoo=-11.887701988220215 -11.637335777282715 52.40430450439453,
3Droo=399.9999800778292,
3Dlights=Headlamp,add3Djscript=asylabels.js}{width=145pt}{.3}{The surface parameterized in Example \ref{ex_parsurf2}.}{fig:parsurf2}{figures/figparsurf2} 
...
In the following two examples, we parameterize the same surface over triangular regions. Each will use $v$ as a ``scaling factor'' as done in Example \ref{ex_parsurf2}.\\
...
{Parameterize the surface $z=x^2+2y^2$ over the triangular region $R$ enclosed by the  lines $y=3-2x/3$, $y=1$ and $x=0$ as shown in Figure \ref{fig:parsurf4}(a). 
}
...
When parameterizing the surface, we can let $x=u$, $a\leq u\leq b$, and we can let $y = g_1(u)+v\big(g_2(u)-g_1(u)\big)$, $0\leq v\leq 1$. The parameterization of $x$ is straightforward, but look closely at how $y$ is determined. When $v=0$, $y=g_1(u) = g_1(x)$. When $v=1$, $y= g_2(u)=g_2(x)$. 
...
{Find a parameterization of the cylinder $x^2 + z^2/4=1$, where $-1\leq y\leq 2$, as shown in Figure \ref{fig:parsurf5}. 
}
...
3Dlights=Headlamp,add3Djscript=asylabels.js}{width=150pt}{.65}{The cylinder parameterized in Example \ref{ex_parsurf5}.}{fig:parsurf5}{figures/figparsurf5} 
...
{Find a parameterization of the elliptic cone $z^2 = \frac{x^2}{4}+\frac{y^2}{9}$, where $-2\leq z\leq 3$, as shown in Figure \ref{fig:parsurf6a}.
}
...
{Find a parameterization of the ellipsoid $\frac{x^2}{25}+y^2+\frac{z^2}{4}=1$ as shown in Figure \ref{fig:parsurf7}(a).
}
...
\example{ex_surface_int05}{Finding the surface area of a parameterized surface}{Determine the surface area of the helicoid (a spiral ramp shown in Figure~\ref{fig:helicoid}) given parametrically by
$$\vec r(u,v) = \left<u\cos v, u\sin v, v\right>$$ for $0 \leq u \leq 1$ and $0 \leq v \leq 4\pi$.}{
\mfigure[scale=0.62]{.79}{Helicoid}{fig:helicoid}{figures/helicoid.jpg}
Notice that we will be integrating over a rectangle in the $uv$-plane. To find the surface area using Theorem \ref{thm:par_surface_area}, we need $\snorm{\vec r_u\times\vec r_v}$.
We find:
\begin{align*}
\vec r_u &= \left< \cos v, \sin v, 0\right>\\
\vec r_v &= \left< -u\sin v, u\cos v, 1\right>\\
\vec r_u\times\vec r_v &= \left< \sin v, -\cos v, u \right>\\
\snorm{\vec r_u\times\vec r_v} &= \sqrt{\sin^2 v + \cos^2 v + u^2} = \sqrt{1+u^2}.
\end{align*}

Integrating over the rectangle in the $uv$-plane gives us the surface area
\begin{align*}
S = \iint_\surfaceS \ dS &= \iint_R\snorm{\vec r_u\times \vec r_v}\ dA \\
& = \int_0^{4\pi} \int_0^1 \sqrt{1+u^2} \: du \: dv \\
&= 4\pi \left(\left( \dfrac{1}{2}\left( u \sqrt{1+u^2} + \ln\left|\sqrt{1+u^2} + u\right| \right)  \right)\right|_0^1\\
&= 2\pi \left( \sqrt{2} + \ln(1 + \sqrt{2}) \right) \text{square units}.
\end{align*}
}\\

\example{ex_parsurfarea1}{Finding the surface area of a parameterized surface}
{Using the parameterization found in Example \ref{ex_parsurf2}, find the surface area of $z=x^2+2y^2$ over the circular disk of radius 2, centered at the origin.
}
{In Example \ref{ex_parsurf2}, we parameterized the surface as $\vec r(u,v) = \la 2v\cos u, 2v\sin u, 4v^2\cos^2u+8v^2\sin^2u\ra$, for $0\leq u\leq 2\pi$, $0\leq v\leq 1$. To find the surface area using Theorem \ref{thm:par_surface_area}, we need $\snorm{\vec r_u\times\vec r_v}$. We find:
\begin{align*}
