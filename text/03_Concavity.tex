...
\item If $\fpp(c)=0$, then the test is inconclusive.
\end{enumerate}
}
...
Both the First Derivative Test and the Second Derivative Test are used to determine if there is a relative maximum, a relative minimum, or neither, at a given critical value.  If one wishes, the First Derivative Test can always be used for this.  The Second Derivative Test, on the other hand, cannot be used to determine any useful information if $\fpp(c)=0$ or if $\fpp(c)$ is undefined (the latter of which includes all critical points for which $\fp(c)$ is undefined).  For example, both $f(x)=x^3$ and $f(x)=x^4$ have a critical number at $c=0$.  For both of these functions, $\fpp(0)=0$ which tells us no information.  In the case of $f(x)=x^3$, there is neither a relative minimum or maximum at $0$, whereas $f(x)=x^4$ has a relative minumum at $x=0$.  Even in cases where the Second Derivative Test can be used, it may be tedious to compute the second derivative a function if not needed.
