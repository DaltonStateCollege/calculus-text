...
\begin{enumerate}
	...
	$$\frac{x-x_0}{a} = \frac{y-y_0}{b}=\frac{z-z_0}{c}$$ when $a,b,c\not=0$.
\end{enumerate}
}
...
Note that the symmetric equations given in Definition~\ref{def:lines} only allow for the case where $a,b,c$ are all nonzero.  This is because of the denominators.  However, we can modify this a bit when one or more of $a,b,c$ are zero.  For example, consider the line whose parametric equations are
$$x = 5+4t,\quad y = 3,\quad z = -2+t.$$  This line lies in the plane $y=3$.  So we can define this line using the equations $$y=3 \text{and }\ds \frac{x-5}{4}=\frac{z+2}{1}$$ together.  The second equation comes from the usual symmetric equations formula for $x$ and $z$.  This pair is called the symmetric equations for this particular line.\\
...
{\begin{enumerate}
	...
	$$h =\frac{\norm{\vv{PQ}\times \vec d}}{\vnorm{d}}.$$
	\item	Let $P_1$ be a point on line $\ell_1$ that is parallel to $\vec d_1$, and let $P_2$ be a point on line $\ell_2$ parallel to $\vec d_2$.  Also, let $\vec c = \vec d_1\times \vec d_2$, where lines $\ell_1$ and $\ell_2$ are not parallel. The distance $h$ between the two lines is:
