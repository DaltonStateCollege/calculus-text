...
{A \textbf{sequence} is a function $a(n)$ whose domain is $\mathbb{N}$ (the set $\{1,2,3,4,\dots\}$ of natural numbers). The \textbf{range} of a sequence is the set of all distinct values of $a(n)$.
...
We define $0! = 1$. This is so that the property $(n+1)!=(n+1)n!$ that holds for $n\geq1$ also holds for $n=0$. This is similar to how the definition of an exponent is extended from positive integers to zero in the natural way.} 
\clearpage
...
\noindent We gave one extra term to begin to show the pattern of signs is ``$-$, $-$, $+$, $+$, $-$, $-$, $\ldots$, due to the fact that the exponent of $-1$ is a special quadratic. Note that the sign is $+$ exactly when $n(n+1)$ is divisible by 4, which is exactly when either $n$ or $n+1$ is divisible by 4. This sequence is plotted in Figure \ref{fig:seq1b}(c).
...
{Let $\{a_n\}$ be a sequence. Then $\ds \lim_{n\to\infty} |a_n| = 0$ if and only if $\ds \lim_{n\to\infty} a_n = 0$\index{Absolute Value Theorem}\index{limit!Absolute Value Theorem}\index{sequence!Absolute Value Theorem}
}
...
\item		$\ds \lim_{n\to\infty} (a_n/b_n) = L/K$ when $K\neq 0$
...
{A sequence $\{a_n\}$ is said to be \textbf{bounded} if there exists real numbers $m$ and $M$ such that $m \leq a_n \leq M$ for all $n$ in $\mathbb{N}$.\\

A sequence $\{a_n\}$ is said to be \textbf{unbounded} if it is not bounded.\\

A sequence $\{a_n\}$ is said to be \textbf{bounded above} if there exists an $M$ such that $a_n \leq M$ for all $n$ in $\mathbb{N}$; it is \textbf{bounded below} if there exists an $m$ such that $m\leq a_n$ for all $n$ in $\mathbb{N}$.
...
{\begin{enumerate}
\item		The terms of this sequence are always positive but are decreasing, so we have $0\leq a_n\leq 1$ for all $n$. Thus this sequence is bounded. Figure \ref{fig:seq3}(a) illustrates this.
...
Sequences are a great source of mathematical inquiry. The On-Line Encyclopedia of Integer Sequences (\url{http://oeis.org}) contains thousands of sequences with formulas when known. (As of this writing, there are 305,558 sequences in the database.  Not all sequences have a known formula.) Perusing this database quickly demonstrates that a single sequence can represent several different ``real life'' phenomena. 
