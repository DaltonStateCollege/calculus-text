...
\item		$\ds \lim_{x\to c} \log_a x = \log_a c$
...
Since the numerator and denominator are each polynomials, we know that $(x-3)$ is factor of each. Using whatever method is most comfortable to you, factor out $(x-3)$ from each (using polynomial division, synthetic division, etc.). We find that $$\frac{x^3-2 x^2-5 x+6}{2 x^3+3 x^2-32 x+15} = \frac{(x-3)(x^2+x-2)}{(x-3)(2 x^2+9 x-5)}.$$ We can cancel the $(x-3)$ terms as long as $x\neq 3$. Using Theorem \ref{thm:limit_allbut1} we conclude:
		\begin{align*}
...
In the following example, the function involves square roots.  We describe an algebra trick used to get the necessary cancellation in order to compute the limit.\\

\example{ex_limit_sqrt_conj}{Evaluating a limit by multiplying numerator and denominator by a conjugate expression}
{Evaluate $\ds \lim_{x\to 1} \frac{\sqrt{3x-2}-\sqrt{x}}{x-1}$.}
{The numerator is not a polynomial, so we cannot factor the expression.  However, to get rid of the ``$0/0$'' problem, we would like to cancel a factor in the numerator and denominator.  The numerator contains square roots, so we use an algebra trick where we first multiply the numerator and denominator by the conjugate of the numerator.  The conjugate of $\sqrt{3x-2}-\sqrt{x}$ is formed by changing the sign between the square roots between '$+$' and '$-$'.  In this case the conjugate is $\sqrt{3x-2}+\sqrt{x}$.  (If the denominator contained square root(s) instead, we would multiply by the denominator's conjugate instead.)  Therefore
\begin{align*}
\lim_{x\to 1} \frac{\sqrt{3x-2}-\sqrt{x}}{x-1} &= \lim_{x\to 1} \frac{\sqrt{3x-2}-\sqrt{x}}{x-1}\cdot \frac{\sqrt{3x-2}+\sqrt{x}}{\sqrt{3x-2}+\sqrt{x}}\\
&=\lim_{x\to 1} \frac{\left(\sqrt{3x-2}-\sqrt{x}\right)\left(\sqrt{3x-2}+\sqrt{x}\right)}{(x-1)\left(\sqrt{3x-2}+\sqrt{x}\right)}\\
&=\lim_{x\to 1} \frac{\left(\sqrt{3x-2}\right)^2-\left(\sqrt{x}\right)^2}{(x-1)\left(\sqrt{3x-2}+\sqrt{x}\right)}\\
&=\lim_{x\to 1} \frac{3x-2-x}{(x-1)\left(\sqrt{3x-2}+\sqrt{x}\right)}\\
&=\lim_{x\to 1} \frac{2x-2}{(x-1)\left(\sqrt{3x-2}+\sqrt{x}\right)}\\
&=\lim_{x\to 1} \frac{2(x-1)}{(x-1)\left(\sqrt{3x-2}+\sqrt{x}\right)}\\
&=\lim_{x\to 1} \frac{2}{\sqrt{3x-2}+\sqrt{x}}\\
&=\lim_{x\to 1} \frac{2}{\sqrt{3(1)-2}+\sqrt{1}}\\
&=\lim_{x\to 1} \frac{2}{1+1}\\
&=1.
\end{align*}
\vskip -\baselineskip
}\\																														
...
&=	\lim_{h\to 0}
\frac{-1.5-3h-1.5 h^2 + 11.5+11.5h - 10}{h}\\
