...
{Suppose $f(x)$ is a continuous function on an interval $I$.  Let $F(x)$ and $G(x)$ be antiderivatives of $f(x)$ on $I$. Then there exists a constant $C$ such that $$G(x) = F(x) + C.$$
}
...
\item $\frac{d}{dx}\big(\tan^{-1} x \big) = \frac{1}{1+x^2}$\myrule
\end{enumerate}
\end{minipage}
