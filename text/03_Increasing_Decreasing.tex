...
{For a function $f$ and interval $I$, suppose\index{First Derivative Test}\index{derivative!First Deriv. Test}\index{extrema!and First Deriv. Test}\index{maximum!and First Deriv. Test}\index{minimum!and First Deriv. Test}
\begin{itemize}
\item $f$ is continuous on $I$,
\item $c$ is a critical number in $I$,
\item $f$ is differentiable on $I$, except possibly at $c$.
\end{itemize}
\begin{enumerate}
...
\item		If the sign of \fp\ does not change at $c$, then $f(c)$ is not a relative extremum of $f$.
\end{enumerate}
}
\normalsize
\restoreboxwidth
...
\mfigure{.7}{A graph of $f(x)$ in Example \ref{ex_incr3}, showing where $f$ is increasing and decreasing.}{fig:incr3}{figures/figincr3}
}\\
